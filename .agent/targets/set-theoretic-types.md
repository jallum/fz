...
## Brands and opaques carry their inner

`brands` and `opaques` are **nominal refinements** over structural representations,
carried as their own axes. The defining property: a nominal type carries its inner
representation **in the same `Descr`**, alongside the tag. A brand `B` declared
`@type B :: refines U` is the structural type `U` with the `brands` axis also set;
an opaque `T` declared `@type T :: opaque U` is `U` with the `opaques` axis set.

```text
mint_brand(binary, "utf8")  : { brands = {utf8}, basic = binary }
plain binary                : { basic  = binary }
opaque(U) for some U        : { opaques = {T}, …U's axes… }
```
...
unbranded type lacks the tag. An opaque is *not* a subtype of its inner: the tag
makes two distinct opaque names lattice-disjoint, and disjoint from the plain inner.

Because the inner lives in the symbol, **every nominal question is answered from the
symbol's own structure** — there is no `brand_inners` / `opaque_inners` side map and
nothing about a name is looked up. `mint_brand(inner, name)` is the constructor that
establishes a brand; it is called once, where the name is defined (see
[`type-naming`](type-naming.md)), so the symbol is complete from birth.
...
Two different questions get two different models. Because the inner travels inside
the symbol, both are decided structurally — no carrier of nominal maps is threaded
into the call:
...
A struct value is modeled as a **nominal opaque tag over a structural field tuple**:
`opaque(impl-target::Range) ∩ {first_type, last_type, step_type}`, with the tuple
slots in `defstruct` order — the inner field tuple living in the same symbol as the
tag. The record type is validated against the `defstruct` schema as a set — every
record field must exist on the struct, every struct field must be named — so field
types are never guessed from constructor sites. Field projection reads the tuple slot
the schema selects when the receiver is a known singleton struct tag; unknown or
ambiguous receivers stay `any`. The nominal tag is the same identity protocol
dispatch uses (see [`protocols`](protocols.md)) and keeps `Range` distinct from any
other three-integer tuple.
