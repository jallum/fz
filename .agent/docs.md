...
- [type-specialization](docs/type-specialization.md) — sidecar type inference over CPS IR: activations are keyed by `FnId` plus input facts, parameters do not default to `any`, proof rides beside visible `Ty`, call targets resolve to arrow sets, operators apply strict signatures, and returns converge by monotone worklist plus finite-height widening.
