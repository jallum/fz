...
- [type-specialization](docs/type-specialization.md) — how the planner infers a function's type per call-contract: parameters start at `any` and the body constrains them, every operation (incl. `+` and `f.(x)`) is a signature application, closures are functions with capture parameters, and specialization is a monotone worklist fixpoint that widens recursive slots in the finite-height refinement lattice; why it terminates.
