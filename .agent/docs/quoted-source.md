...
- `@spec`, `@type`, extern type-position surfaces, and ordinary `::`
  ascription RHS payloads carry compiler2 token payloads produced by the
...
- Compiler2 source ingestion lexes each submitted source once. Type fragments,
  attributes, extern signatures, and `::` RHS payloads are transported as
  quoted token payloads from that pass, so there is no fragment re-lexing or
  quoted-AST token reconstruction under synthetic source names such as
  `<quoted-type-alias>`.
...
- Macro execution lends the owning `QuotedSourceHeap` process to the backend
  ...
  quoted-source heap, runtime-owned pointers such as any transient `ExecCtx` are
  detached again. Macro
