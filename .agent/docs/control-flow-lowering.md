...
`PatternMatrix` through DispatchMatrix into the outer function via
`lower_pattern_matrix_to_current_fn`.
...
`receive` (`receive.rs` `lower_receive`) is DispatchMatrix-driven with a cached
Matcher ABI adapter: the outer function's block ends with `Term::ReceiveMatched`,
whose `clauses` name the body and guard `FnId`s and whose `matcher` is the
cached graph-derived `Arc<Matcher>`. Bodies (`rx_clause_N_
