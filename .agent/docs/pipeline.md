...
- `AbiExecutable(E)` reads `MaterializedExecutable(E)`,
  ...
  boundaries. It retains the materialized product explicitly; materialized
  semantic fields are reached through that field, while ABI layouts and edges
  remain ABI-owned. It does not derive movement from root-wide demand state.
...
each pass indexes the remaining body inventory once, then compares siblings by
