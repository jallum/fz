...
impl<T: Ord + Clone> FiniteSet<T> {
    ...
    /// the answer is `false` where `true` would have been sound.
    ...
    /// case refuses is never part of a chain. `Descr::diff` uses it on the
    /// brand slot, whose domain is infinite (brand names, plus the unbranded
    /// case that no finite set can name), so there the answer is exact.
