...
pub struct ModuleTypes {
    ...
    pub any_key_specs: HashMap<FnId, Vec<crate::types_seam::Ty>>,
...
impl ModuleTypes {
    ...
    pub fn effective_return_for_call(&self, callee: FnId, arg_descrs: &[Descr]) -> Option<Descr> {
        ...
        self.effective_return_for_call_ty(callee, &arg_tys)
            .map(|t| t.descr().clone())
    }
...
pub fn type_module<T: crate::types_seam::Types>(t: &mut T, m: &Module) -> ModuleTypes {
    ...
    let mut work: std::collections::VecDeque<(FnId, Vec<crate::types_seam::Ty>)> =
        ...
        for b in &f.blocks {
            ...
            for key in keys {
                ...
                for stmt in &b.stmts {
                    ...
                    let pt = crate::types_seam::Ty::from_descr(t.to_descr(&pt_ty));
                    env.insert(*v, pt);
                }
                ...
                let ct = env
                    .get(&cond)
                    .cloned()
                    .unwrap_or_else(|| crate::types_seam::Ty::from_descr(any_d.clone()));
        ...
        *count += 1;
        ...
        if changed {
            effective_returns.insert(
                spec_key.clone(),
                crate::types_seam::Ty::from_descr(t.to_descr(&new_ret)),
            );
        ...
        match &b.terminator {
            ...
            Term::TailCallClosure {
                ...
                if let Some(&target) = ft.fn_constants.get(closure) {
                    ...
                    let mut ad: Vec<crate::types_seam::Ty> =
                        args.iter()
                            .map(|av| {
                                ft.vars.get(av).cloned().unwrap_or_else(|| {
                                    crate::types_seam::Ty::from_descr(any_d.clone())
                                })
                            })
                            .collect();
                    while ad.len() < np {
                        ad.push(crate::types_seam::Ty::from_descr(any_d.clone()));
                    }
                    ad.truncate(np);
                    let key = (target, ad);
                    let d = effective_returns.get(&key);
                    ...
                    if let Some(view) = funcs_view {
                        if view.has_negations() || !view.all_clauses_have_pos() {
                            ...
                            for arrow in view.arrows() {
                                ...
                                let mut full_key: Vec<crate::types_seam::Ty> = lit.captures.clone();
                                for av in args.iter() {
                                    full_key.push(ft.vars.get(av).cloned().unwrap_or_else(|| {
                                        crate::types_seam::Ty::from_descr(any_d.clone())
                                    }));
                                }
                                while full_key.len() < np {
                                    full_key.push(crate::types_seam::Ty::from_descr(any_d.clone()));
                                }
            ...
            Term::ReceiveMatched {
                ...
                for c in clauses {
                    ...
                    let mut key: Vec<crate::types_seam::Ty> =
                        vec![crate::types_seam::Ty::from_descr(any_d.clone()); c.bound_names.len()];
                    key.extend(cap_tys.iter().cloned());
                    while key.len() < np {
                        key.push(crate::types_seam::Ty::from_descr(any_d.clone()));
                    }
                    key.truncate(np);
        ...
        Term::CallClosure { closure, args, .. } => {
            if let Some(&target) = ft.fn_constants.get(closure) {
                ...
                resolve_closure_return_tys(t, cv_descr.descr(), effective_returns, &arg_tys)
                    .map(|ty| Ty::from_descr(t.to_descr(&ty)))
                    .unwrap_or_else(|| any_t.clone())
            } else {
                any_t.clone()
            }
...
fn walk_spec_for_discovery<T: crate::types_seam::Types>(
    ...
    let any_d = Descr::any();
    let any_ty = crate::types_seam::Ty::from_descr(any_d.clone());
    fn widen_direct<T: crate::types_seam::Types>(
        t: &mut T,
        widen_now: bool,
        caller_scc: &std::collections::HashSet<FnId>,
        k: Vec<crate::types_seam::Ty>,
        callee: FnId,
    ) -> Vec<crate::types_seam::Ty> {
    ...
    for b in &f.blocks {
        ...
        for stmt in b.stmts.iter() {
                ...
                && let Some(&jj) = m.fn_idx.get(lam_fn_id)
            {
                ...
                let captures: Vec<crate::types_seam::Ty> = captured
                    .iter()
                    .map(|cv| {
                        env.get(cv)
                            .cloned()
                ...
                let any_key: Vec<crate::types_seam::Ty> = vec![any_ty.clone(); n_params];
            ...
            {
                let pt_ty = type_prim(t, prim, &env, m, &HashSet::new());
                env.insert(*v, crate::types_seam::Ty::from_descr(t.to_descr(&pt_ty)));
            }
        }
        ...
        for BlockCallsite { slot, kind } in cs_list {
            match kind {
                CallsiteKind::Direct { callee, args } => {
                    ...
                    let mut dispatch_key: Vec<crate::types_seam::Ty> = args
                        .iter()
                ...
                CallsiteKind::Cont { cont, source } => {
                    ...
                    let slot0_ty: Option<crate::types_seam::Ty> = match source {
                    ...
                    let cont_fn = &m.fns[j];
                    let n_params = cont_fn.block(cont_fn.entry).params.len();
                    let mut key: Vec<crate::types_seam::Ty> = vec![any_ty.clone(); n_params];
                    if !key.is_empty() {
                        key[0] = slot0;
                    }
        ...
        } = &b.terminator
        {
            ...
            let mut enq = |fid: FnId, bound_arity: usize, ident: crate::fz_ir::CallsiteIdent| {
                ...
                let mut key: Vec<crate::types_seam::Ty> = vec![any_ty.clone(); bound_arity];
                key.extend(cap_tys.iter().cloned());
                while key.len() < np {
...
pub fn type_fn<T: crate::types_seam::Types>(
    ...
    let mut block_envs: HashMap<BlockId, HashMap<Var, crate::types_seam::Ty>> = HashMap::new();
                ...
                Term::Goto(target, args) => {
                    ...
                    for &p in target_b.params.iter() {
                        let from_env = block_envs[target]
                            .get(&p)
                            .cloned()
                            .unwrap_or_else(|| crate::types_seam::Ty::from_descr(Descr::none()));
...
fn merge_into<T: crate::types_seam::Types>(
    ...
    target: BlockId,
    delta: &HashMap<Var, crate::types_seam::Ty>,
) -> bool {
    ...
    for (v, dt) in delta {
        ...
        if !t.is_equivalent(&unioned, &prev_ty) {
            env.insert(*v, crate::types_seam::Ty::from_descr(t.to_descr(&unioned)));
            changed = true;
        }
    }
    changed
...
fn union_envs<T: crate::types_seam::Types>(
    ...
    for (v, dt) in b {
        ...
        out.insert(*v, crate::types_seam::Ty::from_descr(t.to_descr(&unioned)));
    }
    out
}
...
fn narrow_for_cond<T: crate::types_seam::Types>(
    ...
    match prim {
        ...
        Prim::IsEmptyList(v) => {
            ...
            else_env.insert(*v, crate::types_seam::Ty::from_descr(t.to_descr(&else_t)));
        }
        Prim::BinOp(BinOp::Eq, a, b) => {
            ...
            if t.is_singleton_lit(&at) {
                ...
                then_env.insert(*b, crate::types_seam::Ty::from_descr(t.to_descr(&then_b)));
                else_env.insert(*b, crate::types_seam::Ty::from_descr(t.to_descr(&else_b)));
            }
            if t.is_singleton_lit(&bt) {
                let then_a = t.intersect(at.clone(), bt.clone());
                let else_a = t.difference(at.clone(), bt.clone());
                then_env.insert(*a, crate::types_seam::Ty::from_descr(t.to_descr(&then_a)));
                else_env.insert(*a, crate::types_seam::Ty::from_descr(t.to_descr(&else_a)));
            }
        }
...
pub fn collect_diagnostics<T: crate::types_seam::Types>(
    ...
    //
    // Group specs by FnId.
    let mut specs_by_fn: HashMap<crate::fz_ir::FnId, Vec<Vec<crate::types_seam::Ty>>> =
        HashMap::new();
    ...
    for f in &module.fns {
        ...
        let any_key_ty: Vec<crate::types_seam::Ty> =
            vec![crate::types_seam::Ty::from_descr(Descr::any()); n_params];
...
pub fn reachable_specs<T: crate::types_seam::Types>(
    ...
    if let Some(main_fn) = module.fns.iter().find(|f| f.name == "main") {
        ...
        let key: Vec<crate::types_seam::Ty> =
            vec![crate::types_seam::Ty::from_descr(Descr::any()); n_params];
    ...
    while let Some(sid) = worklist.pop() {
        ...
        for blk in &f.blocks {
            ...
            let any_ty = crate::types_seam::Ty::from_descr(Descr::any());
            ...
            let pad_to_arity =
                |callee: FnId, mut tys: Vec<crate::types_seam::Ty>| -> Vec<crate::types_seam::Ty> {
...
pub fn cont_input_key<T: crate::types_seam::Types>(
    ...
    let n_params = cont_fn.block(cont_fn.entry).params.len();
    let any_t = Ty::from_descr(Descr::any());
    let mut key: Vec<Ty> = vec![any_t.clone(); n_params];
    if !key.is_empty() {
        let slot0_ty = cont_slot0_descr(t, block, caller_ft, module, module_types);
        key[0] = Ty::from_descr(t.to_descr(&slot0_ty));
    }
    let env = env_at_terminator(t, caller_ft, block, module);
