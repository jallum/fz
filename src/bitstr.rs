...
fn resolve_size(spec: &BitFieldSpec, env: &Env) -> Result<Option<u32>, String> {
    Ok(match &spec.size {
        ...
        Some(BitSize::Var(name)) => match env.lookup(name) {
            ...
            Some(other) => {
                return Err(format!(
                    "size variable `{}` must be a non-negative int, got {}",
                    name, other
                ));
            }
...
fn encode_integer(
    ...
    if total > 64 {
        return Err(format!("integer field too wide: {} bits", total));
    }
...
fn encode_binary(
    ...
    if total_bits > bytes_rc.len() * 8 {
        return Err(format!(
            "binary field size {} exceeds available {} bits",
            total_bits,
            bytes_rc.len() * 8
        ));
    }
...
fn encode_bits(
    ...
    if total_bits > bit_len {
        return Err(format!(
            "bits field size {} exceeds available {} bits",
            total_bits, bit_len
        ));
    }
    let mut r = BitReader {
    ...
    for (i, f) in fields.iter().enumerate() {
        ...
        let extracted: Value = match f.spec.ty {
            BitType::Integer => {
                ...
                let raw = match reader.read_bits(total as usize) {
                    Some(v) => v,
                    None => return false,
                };
                let raw = apply_endian_for_read(raw, total, f.spec.endian);
                let n = if f.spec.signed {
            ...
            BitType::Utf8 => match decode_utf8(&mut reader) {
                Some(c) => Value::Int(c as i64),
                None => return false,
            },
            BitType::Utf16 => match decode_utf16(&mut reader, f.spec.endian) {
                Some(c) => Value::Int(c as i64),
                None => return false,
            },
            BitType::Utf32 => match decode_utf32(&mut reader, f.spec.endian) {
                Some(c) => Value::Int(c as i64),
                None => return false,
            },
        };
        if !match_pattern(&f.value.node, &extracted, env) {
            return false;
        }
    }
    reader.remaining() == 0
