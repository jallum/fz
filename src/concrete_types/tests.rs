...
use super::*;
use crate::types::{ClosureTypes, MapKey, Nominals, SchemeInstantiation, Types};
...
fn scheme_result_widens_reduce_while_accumulator_from_reducer_exits() {
    ...
    let SchemeInstantiation::Known(acc) = instantiated else {
...
fn scheme_result_rejects_structural_witness_mismatch() {
    ...
    let constraints = std::collections::HashMap::new();
    let instantiated =
        crate::types::instantiate_scheme_result(&mut t, &[param], &a, &constraints, &[witness]);
