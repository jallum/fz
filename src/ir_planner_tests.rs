...
"#,
    ...
    let caller_ft = mt.specs.get(&value_spec_key(main.id, vec![])).unwrap();
    ...
    for blk in &main.blocks {
        if let Term::Call { continuation, .. } = &blk.terminator {
            ...
            assert!(
                mt.specs
                    .contains_key(&value_spec_key(continuation.fn_id, key.clone())),
                ...
                mt.specs
                    .iter()
                    .filter(|(key, _)| key.fn_id == continuation.fn_id)
                    .map(|(key, _)| crate::types::display_key_slots(&t, &key.input))
                    .collect::<Vec<_>>(),
            );
            found_any = true;
        }
...
/// fz-loop.4 — the rendered resume is backed by a structured planner helper,
/// not string-only dump reconstruction. Source clauses lower through
/// `fn_clause_*`, so this pins the resume target and result-threading without
/// requiring the reducer call to live directly in the source fn's body.
#[test]
fn recognizes_callthen_resume_state_update() {
    let src = "fn reduce_list([], {:cont, acc}, _reducer), do: {:done, acc}\n\
               fn reduce_list([h | t], {:cont, acc}, reducer), do: reduce_list(t, reducer(h, acc), reducer)\n\
               fn main(), do: reduce_list([1, 2], {:cont, 0}, fn (x, acc) -> {:cont, acc + x})";
    let (_t, m, _mt) = pipeline(src, &crate::telemetry::NullTelemetry);
    let reduce_list = m.fn_by_name("reduce_list").expect("reduce_list fn");
    let update = m
        .fns
        .iter()
        .flat_map(|f| f.blocks.iter())
        .find_map(|block| {
            if let Term::CallClosure { continuation, .. } = &block.terminator {
                crate::ir_planner::state_transitions::continuation_resume_state_update(
                    &m,
                    continuation,
                )
            } else {
        ...
        .expect("reducer continuation should have a resume state update");
    assert_eq!(update.target, reduce_list.id);
    assert!(
        update.args.iter().any(|arg| {
            matches!(
                arg,
                crate::ir_planner::state_transitions::ResumeStateArg::Result
            )
        }),
        "state update should thread the reducer result into the next reduce_list call: {:?}",
        update
    );
}

#[test]
fn no_resume_state_update_for_non_tail_continuation() {
    let src = "fn map(_, []), do: []\n\
               fn map(f, [h | t]), do: [f(h) | map(f, t)]\n\
               fn double(x), do: x * 2\n\
               fn main(), do: map(double, [1, 2])";
    let (_t, m, _mt) = pipeline(src, &crate::telemetry::NullTelemetry);
    let has_update = m
        .fns
        .iter()
        ...
        .any(|update| update.target == m.fn_by_name("map").expect("map fn").id);
    assert!(
        !has_update,
        "map's reducer-like call feeds cons construction, not an immediate tail resume"
    );
}
