...
impl RawProductKey {
    ...
    fn canonical(&self, canon: &CanonTables) -> String {
        canonical_identity(&self.raw, canon)
    }
}
...
fn canonical_identities(
    ...
    let mut remember_product = |product: &RawProductKey| {
        products
            .entry(product.clone())
            .or_insert_with(|| product.canonical(&report.canon));
    };
...
struct SessionReplay {
    ...
    requests: HashMap<RawProductKey, usize>,
...
impl Replay {
    ...
    fn process(&mut self, position: usize, event: &PublicEvent) {
        ...
        if event.named(BACKEND_REQUEST_STARTED) {
            ...
            if let Some(program) = event.metadata.get("program") {
                self.report.final_population.reachable_executables =
                    program.get("executables").and_then(Json::as_u64).unwrap_or(0);
                self.report.final_population.construction_wrappers =
                    program.get("construction_wrappers").and_then(Json::as_u64).unwrap_or(0);
            }
    ...
    fn request_product(&mut self, position: usize, event: &PublicEvent) {
        ...
        let key = RawProductKey::new(product);
        session.requests.insert(key.clone(), position);
    ...
    fn evaluate_product(&mut self, position: usize, event: &PublicEvent) {
        ...
        let started = session.requests[&product];
...
fn canonical_identity(value: &Json, canon: &CanonTables) -> String {
    render_identity(&identity_value(value, Some(canon)))
}
