    ...
    match &e.node {
        ...
        Expr::Call(target, args) => {
            ...
            if callee_name == "spawn" && (arg_vars.len() == 1 || arg_vars.len() == 2) {
                ...
                let sym = if arg_vars.len() == 1 { "fz_spawn" } else { "fz_spawn_opt" };
                let eid = ctx.externs.lookup(sym).expect("fz_spawn/fz_spawn_opt must be in runtime.fz");
...
mod tests {
    ...
    fn lower_lambda_creates_separate_fn_and_closure() {
        ...
        assert!(m.fns.len() >= 2, "expected ≥2 fns (mk + lambda + prelude), got {}", m.fns.len());
        assert!(
            m.fns.iter().any(|f| f.name == "mk"),
            "expected 'mk' fn"
        );
        assert!(
...
"#,
    ...
    fn extern_fn_registers_in_module_externs() {
        ...
        let nop = module.externs.iter().find(|e| e.fz_name == "fz_nop")
