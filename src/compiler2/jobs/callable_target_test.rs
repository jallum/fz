//! `DeriveCallableConstructionTarget` read through the fact it publishes.
//!
//! A construction target is the exact activation a first-class callable is
//! built for, so the statements here are about what that target frame is and
//! how the fact behaves when the owner it belongs to appears, changes and
//! goes away.
...
fn construction_target_is_exact_memoized_and_read_without_formula_minting() {
    ...
    crate::compiler2::product_drive::drive_retained_root_backend_product(&mut world, &tel, &mut sessions, root, None)
    ...
    // The frame length above is only the right length if the function it
    // belongs to is the lambda. The lambda is generated while `main` is
    // lowered, and it used to be handed `main`'s quoted root, so a later
    // source job could derive `main`'s surface and install it here -- leaving
    // a one-input frame in front of a zero-arity surface.
    let lambda = target.activation.function;
    let (lambda_source, lambda_surface) = world.function_definition(lambda);
    assert_eq!(
        lambda_surface.arity(),
        key.surface.inputs.len(),
        "the target's function publishes its own surface, not the surface of the function that minted it",
    );
    assert_eq!(
        lambda_source.capture_params.len(),
        target_inputs.len() - key.surface.inputs.len(),
        "the capture prefix of the frame is exactly what the lambda's own source says it captured",
    );
    assert_eq!(
        &target_inputs[1..],
    ...
    // Main no longer mints a lambda, so the owner's derived facts and the
    // source now disagree: the facts still name a callable at a function the
    // source does not produce. Handing those stale facts back is the sharpest
    // way to ask who may define that function.
    //
    // The answer is: only main's lowering, and it does not define it any more.
    // The drive stalls, honestly, on the lambda's missing definition. What it
    // must NOT do is invent one -- which is what the old code did, by handing
    // the lambda main's quoted root so a source job could derive main's
    // surface and publish it under the lambda's id.
    ...
    let outcome = ExecutionContext::with_product_sessions(&mut world, &tel, &mut sessions).drive();
    let DriveOutcome::Unresolved { waits } = outcome else {
        panic!("a target whose function the source no longer mints cannot settle, got {outcome:?}");
    };
    let lambda_definition = waits
        .iter()
        .find(|wait| wait.fact.fact().fact() == Some(&FactKey::FunctionDefined(lambda)))
        .expect("the stall names the lambda's own definition as what is missing");
    assert!(
        !lambda_definition.jobs.contains(&Job::DefineFunction(lambda))
            && !lambda_definition.jobs.contains(&Job::ExpandFunctionSource(lambda)),
        "no source job may wait on a generated function's definition, got {:?}",
        lambda_definition.jobs,
    );
    assert!(
        world.callable_construction_target(&key).is_none(),
        "the target stays retracted: nothing may republish it from a definition that no longer exists",
    );
}
