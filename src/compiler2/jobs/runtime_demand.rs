...
fn settle_demand_cone<T: Telemetry>(
    ...
    loop {
        ...
        let mut own_inputs = HashMap::new();
        for member in &members {
        ...
        for member in &members {
            ...
            let own = own_inputs
                .remove(member)
                .expect("every cone member must have one current-round own-demand overlay");
...
fn finish_callable_flows(
    ...
    let answers = input
        ...
        .collect::<HashMap<_, _>>();
    for plan in plans {
        let first_class_edges = dispatch_stress::perturbed_construction_members(
