...
pub(crate) struct NativeProgramMap {
    slots: Vec<ProjectionState<Rc<NativeProgram>>>,
}
...
impl<T> RootProjectionMap<T> {
    fn define_by(&mut self, root: RootId, value: T, same: impl FnOnce(&T, &T) -> bool) -> bool {
        ...
        let changed = match slot {
            ProjectionState::Placeholder => true,
            ProjectionState::Defined(previous) => !same(previous, &value),
        };
...
fn native_program_same_state(
    left: &ProjectionState<Rc<NativeProgram>>,
    right: &ProjectionState<Rc<NativeProgram>>,
) -> bool {
    match (left, right) {
