...
impl<'s> ProductReadContext<'s> {
    ...
    /// Add the prospective read to a borrowed graph view, then use one
    /// traversal to decide whether it closes a recursive group. Discovery must
    /// precede stale-read normalization: the pending edges are the evidence
    /// that the products are waiting on one another.
    pub(crate) fn read_recursive_product(
        ...
        let mut prospective = self.dependencies.clone();
        prospective
            .products
            .insert(dependency.clone(), self.session.memo.generation(&dependency));
        let (members, search) = self
            .session
            .memo
            .pending_strong_component(current, &prospective, &dependency, types);
...
mod tests {
    ...
    fn recursive_group_search_work_is_a_function_of_the_pending_graph() {
        ...
        for _ in 0..32 {
            ...
            let (members, search) = memo.pending_strong_component(
                &current,
                &ProductDependencies {
                    products: HashMap::from([(target.clone(), None)]),
                    facts: HashMap::new(),
                },
                &target,
                &types,
    ...
    fn recursive_group_search_matches_pending_graph_boundaries() {
        ...
        let overlay = |dependency: &ProductKey| ProductDependencies {
        ...
        assert_eq!(
            missing.pending_strong_component(&current, &overlay(&dependency), &dependency, &types),
            (
                None,
                RecursiveGroupSearch {
                    candidate_inventory: 0,
        ...
        assert_eq!(
            self_cycle.pending_strong_component(&current, &overlay(&current), &current, &types),
            (
        ...
        assert_eq!(
            disjoint.pending_strong_component(&current, &overlay(&dependency), &dependency, &types),
            (
                None,
                RecursiveGroupSearch {
        ...
        let (members, search) =
            cross_kind.pending_strong_component(&current, &overlay(&dependency), &dependency, &types);
        assert_eq!(
            members.map(|members| members.into_iter().collect::<HashSet<_>>()),
        ...
        finish_test_product(&mut session.memo, &dependency, ProductValue::Unit, []);
        let mut context = ProductReadContext::new(&mut session);
        assert!(matches!(
            context.read_recursive_product(&tel, dependency, &current, &types),
