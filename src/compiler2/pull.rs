...
impl<'s> ProductReadContext<'s> {
    ...
    pub(crate) fn read_recursive_product(
        ...
        let (members, search) = self
            .session
            .memo
            .pending_strong_component(current, &prospective, &dependency);
...
mod tests {
    ...
    fn recursive_group_search_matches_pending_graph_boundaries() {
        ...
        assert_eq!(
            missing.pending_strong_component(&current, &overlay(&dependency), &dependency),
            (
                None,
                RecursiveGroupSearch {
                    candidate_inventory: 0,
        ...
        assert_eq!(
            self_cycle.pending_strong_component(&current, &overlay(&current), &current),
            (
        ...
        assert_eq!(
            disjoint.pending_strong_component(&current, &overlay(&dependency), &dependency),
            (
                None,
                RecursiveGroupSearch {
        ...
        let (members, search) = cross_kind.pending_strong_component(&current, &overlay(&dependency), &dependency);
        assert_eq!(
            members.map(|members| members.into_iter().collect::<HashSet<_>>()),
        ...
        assert!(matches!(
            context.read_recursive_product(&tel, dependency, &current),
