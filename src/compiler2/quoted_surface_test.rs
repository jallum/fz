...
use super::quoted_surface::{ScopeForm, read_protocol_impl_body_surface, read_scope_surface};
...
fn compiler2_quoted_surface_groups_multiclause_functions_like_legacy_items() {
    ...
    let surface_again = read_scope_surface(&root, &program.items, &program.attrs).expect("surface reread");
        ...
        ScopeForm::ProtocolImpl(form) => {
            ...
            match &body.forms[0] {
                ScopeForm::Function(function) => {
                    ...
                    assert_eq!(function.legacy_fn.clauses.len(), 2);
                }
