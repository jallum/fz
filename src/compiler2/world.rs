...
impl World {
    ...
    pub fn field_layouts(&self, shape: ShapeId) -> Option<&[TransportLayout]> {
        self.transport.interners().field_layouts(shape)
    }
    ...
    pub fn field_spans(
        ...
        self.transport.interners().field_spans(shape)
    }
...
enum FunctionSurfaceMatch {
    ...
    Opaque,
    /// A plain `def`/`defp`/`defmacro`/`defmodule`/`defprotocol`/`defimpl` head
...
/// The span of `function_ref`'s matching `def`/`defp`/`defmacro` form in
...
/// the home of `function_ref`. A reserved head (`def`/`defp`/`defmacro`/
