...
fn run_dump(tel: &ConfiguredTelemetry, args: &[String]) {
    ...
    if emit_specs {
        ...
        let dump = compiler.dump_specs_text(tel, &sm_cell, src, path.clone());
    ...
    if emit_interfaces {
        ...
        let dump = compiler.dump_interfaces_text(tel, &sm_cell, src, path.clone(), strict_interfaces);
    ...
    if emit_stats {
        ...
        let _ = compiler.compile_pipeline("fz dump", tel, &sm_cell, src, path.clone(), mode);
        return;
    }
    ...
    let compiled = compiler.compile_pipeline("fz dump", tel, &sm_cell, src, path.clone(), mode);
...
impl Compiler {
    fn frontend(&mut self, src: String, source_name: String, tel: &dyn Telemetry) -> FrontendResult {
...
fn run_jit_src(tel: &ConfiguredTelemetry, src: String, source_name: String, mode: CompileMode) {
    ...
    tel.attach(&["fz", "diag"], Box::new(DiagRenderer::new_stderr(sm_cell.clone())));
    let mut compiler = Compiler::new();
    let compiled = compiler.compile_pipeline("fz run", tel, &sm_cell, src, source_name, mode);
