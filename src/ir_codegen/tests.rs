...
#[test]
#[serial_test::serial]
fn variadic_open_jit_creates_file_with_mode_bits() {
    use std::ffi::CString;
...
#[test]
fn unsupported_variadic_extern_shape_is_codegen_error() {
    let m = lower_src(
        r#"
...
"#,
    ...
    let err = match compile(
        ...
        Ok(_) => panic!("unsupported variadic shape should fail codegen"),
        Err(err) => err,
    };
    assert!(err.message.contains("unsupported variadic extern shape"));
