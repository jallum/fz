...
fn any_value_from_heap_object_word(bits: u64) -> AnyValue {
    AnyValue::decode_tagged_heap_bits(bits)
...
fn any_value_ref_from_word(word: u64, context: &str) -> AnyValueRef {
    AnyValueRef::from_raw_word(word).unwrap_or_else(|err| panic!("{context}: invalid any value ref {word:#x}: {err:?}"))
}

fn any_value_from_ref_word(word: u64, context: &str) -> AnyValue {
    ...
    match value.tag() {
        ...
        ValueKind::STRUCT => AnyValue::heap_ptr(value.struct_addr().expect(context), ValueKind::STRUCT),
        ValueKind::CLOSURE => AnyValue::heap_ptr(value.closure_addr().expect(context), ValueKind::CLOSURE),
        ValueKind::BITSTRING => AnyValue::heap_ptr(value.bitstring_addr().expect(context), ValueKind::BITSTRING),
        ValueKind::PROCBIN => AnyValue::heap_ptr(value.procbin_addr().expect(context), ValueKind::PROCBIN),
        ValueKind::RESOURCE => AnyValue::heap_ptr(value.resource_addr().expect(context), ValueKind::RESOURCE),
...
fn closure_addr_from_ref_word(word: u64, context: &str) -> *mut u8 {
    any_value_ref_from_word(word, context).closure_addr().expect(context)
}

fn map_ref_word_from_bits(bits: u64) -> u64 {
    let addr = map_addr_from_tagged(bits).expect("map heap bits");
...
fn alloc_stat_entries(process: *mut Process, entries: &mut Vec<(AnyValue, AnyValue)>, prefix: &str, stat: AllocStat) {
    let allocs_key = process_atom_id(process, &format!("{prefix}_allocs"));
    let bytes_key = process_atom_id(process, &format!("{prefix}_bytes"));
    entries.push((AnyValue::atom(allocs_key), AnyValue::int(stat.allocs as i64)));
    entries.push((AnyValue::atom(bytes_key), AnyValue::int(stat.bytes as i64)));
}
...
pub extern "C" fn fz_process_heap_alloc_stats(process: *mut Process) -> u64 {
    ...
    entries.push((
        AnyValue::atom(process_atom_id(process, "bytes")),
        AnyValue::int(snapshot.total.bytes as i64),
    ));
    ...
    entries.push((
        AnyValue::atom(process_atom_id(process, "scheduler_yields")),
        AnyValue::int(scheduler_yields as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "interpreter_yields")),
        AnyValue::int(interpreter_yields as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "reductions_remaining")),
        AnyValue::int(reductions_remaining as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "reductions_per_quantum")),
        AnyValue::int(reductions_per_quantum as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "reductions_executed")),
        AnyValue::int(reductions_executed as i64),
    ));
    entries.push((
    ...
    entries.push((
        ...
        AnyValue::int(yield_reasons as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "max_yield_continuation_bytes")),
        AnyValue::int(max_yield_continuation_bytes as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "min_yield_continuation_margin_before_bytes")),
        AnyValue::int(min_yield_continuation_margin_before_bytes as i64),
    ));
    entries.push((
        AnyValue::atom(process_atom_id(process, "min_yield_continuation_margin_after_bytes")),
        AnyValue::int(min_yield_continuation_margin_after_bytes as i64),
    ));
...
fn map_bits_from_ref_word(word: u64, context: &str) -> u64 {
    ...
    heap_object_word(map.map_addr().expect(context), ValueKind::MAP)
}
...
pub extern "C" fn fz_struct_schema_id_ref(ref_word: u64) -> u32 {
    ...
    unsafe { struct_schema_id(addr.cast_const()) }
}
...
pub extern "C" fn fz_truthy_ref(ref_word: u64) -> u8 {
    ...
    (atom != FALSE_ATOM_ID as u64 && atom != NIL_ATOM_ID as u64) as u8
}
...
pub extern "C" fn fz_dbg_value_ref(process: *mut Process, ref_word: u64) {
    ...
    emit_print_line(process, render_value(process, value));
}
...
fn halt_value_from_slot(value: AnyValue) -> i64 {
    match value.kind() {
...
#[inline]
unsafe fn process_ctx<'a>(process: *mut Process) -> &'a ExecCtx {
...
pub extern "C" fn fz_receive_park_matched(
    ...
    let matcher_fn: MatcherFn = unsafe { transmute(matcher_fn_bits as usize) };
    ...
    let pinned: Vec<AnyValueRef> = if n_pinned == 0 {
        Vec::new()
    } else {
        unsafe { from_raw_parts(pinned_ptr, n_pinned as usize).to_vec() }
    };
    let clause_bodies: Vec<*mut u8> = if n_clauses == 0 {
        Vec::new()
    } else {
        unsafe {
            from_raw_parts(clause_bodies_ptr, n_clauses as usize)
                .iter()
                .map(|&bits| closure_addr_from_ref_word(bits, "fz_receive_park_matched clause body"))
                .collect()
        }
    };
    let clause_bound_counts: Vec<u16> = if n_clauses == 0 {
        ...
        unsafe { from_raw_parts(clause_bound_counts_ptr, n_clauses as usize) }
            .iter()
            .map(|&n| n as u16)
            .collect()
    };
    ...
    let after_timer_id = match after_deadline_ms {
        Some(after_ms) => timer_schedule(p, p.pid, after_ms),
        None => None,
    };
...
pub extern "C" fn fz_yield_mid_flight_report(
    ...
    let closure_addr = closure_addr_from_ref_word(cont_closure_bits, "fz_yield_mid_flight_report cont");
    let closure_bits = heap_object_word(closure_addr, ValueKind::CLOSURE);
    let continuation_bytes = object_size(closure_bits);
...
pub extern "C" fn fz_get_halt_cont(process: *mut Process, halt_cont_body_addr: u64, kind: u32) -> u64 {
    ...
    heap_ref_word(ValueKind::CLOSURE, p.halt_cont_singletons[slot] as *const u8)
}
...
pub extern "C" fn fz_bs_begin(process: *mut Process) {
    (unsafe { &mut *process }).bs_builder = Some(BitWriter::new());
}

/// Write one field into the active builder. Field-type tags match the order
/// in `BitType`: Integer=0, Float=1, Binary=2, Bits=3, Utf8=4,
    ...
    let _ = signed;
    {
        ...
        match ty {
            BitType::Integer => {
                ...
                let bswap = apply_endian_for_write(masked, total, endian);
            ...
            BitType::Binary | BitType::Bits => {
                ...
                let p =
                    bitstring_like_ptr(bits).unwrap_or_else(|| panic!("binary/bits bit field expects heap bitstring"));
                if !unsafe { is_bitstring_like(p) } {
                    panic!("binary/bits bit field source is not a Bitstring");
                }
                let src_bit_len = unsafe { bitstring_bit_len(p) } as usize;
                let src_bytes_ptr = unsafe { bitstring_byte_ptr(p) };
                ...
                let src_bytes = unsafe { from_raw_parts(src_bytes_ptr, src_bit_len.div_ceil(8)) };
            ...
            BitType::Utf8 | BitType::Utf16 | BitType::Utf32 => {
                ...
                let bytes = match ty {
                    BitType::Utf8 => encode_utf8(cp),
                    BitType::Utf16 => encode_utf16(cp, endian),
                    BitType::Utf32 => encode_utf32(cp, endian),
                    _ => unreachable!(),
                };
...
fn byte_aligned_binary_slice(word: u64, context: &str) -> (*const u8, usize) {
    ...
    assert!(unsafe { is_bitstring_like(p) }, "{context}: expected binary");
...
pub extern "C" fn fz_binary_concat(process: *mut Process, left_ref: u64, right_ref: u64) -> u64 {
    ...
    let left = unsafe { from_raw_parts(left_ptr, left_len) };
    let right = unsafe { from_raw_parts(right_ptr, right_len) };
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_alloc_bitstring_const(process: *mut Process, ptr: u64, byte_len: u64, bit_len: u64) -> u64 {
...
fn decode_bit_type(t: u32) -> BitType {
    match t {
        0 => BitType::Integer,
        1 => BitType::Float,
        2 => BitType::Binary,
...
pub extern "C" fn fz_bs_reader_init_ref(process: *mut Process, bs_ref: u64) -> u64 {
    fz_bs_reader_init_bits(process, heap_object_word_from_ref_word(bs_ref, "fz_bs_reader_init_ref"))
}
...
fn fz_bs_reader_init_bits(process: *mut Process, bs_bits: u64) -> u64 {
    ...
    let bit_len = unsafe { bitstring_bit_len(p) } as i64;
...
pub extern "C" fn fz_bs_read_field_ref(
    ...
    let (ty_tag, size_present, unit, endian_tag, signed, is_last) = decode_bs_field_spec(field_spec);
...
pub extern "C" fn fz_bs_reader_done_ref(process: *mut Process, reader_ref: u64) -> u8 {
    ...
    let bit_len = (unsafe { &mut *process }).heap.read_field_slot(reader, 8).raw();
    let pos = (unsafe { &mut *process }).heap.read_field_slot(reader, 16).raw();
    (bit_len == pos) as u8
}
...
fn fz_bs_read_field_bits(
    ...
    // Decode reader tuple.
    let rp =
        struct_addr_from_tagged(reader_bits).unwrap_or_else(|| panic!("read_field: reader is not a tagged Struct"));
    ...
    let pos = (unsafe { &mut *process }).heap.read_field_slot(rp, 16).raw() as usize;

    // Bytes pointer from bs.
    let bsp = bitstring_like_ptr(bs_bits).expect("read_field: reader bs not a ptr");
    if !unsafe { is_bitstring_like(bsp) } {
        panic!("read_field reader bs is not a Bitstring");
    }
    let bytes_ptr = unsafe { bitstring_byte_ptr(bsp) };
    let bytes = unsafe { from_raw_parts(bytes_ptr, bit_len.div_ceil(8)) };
    ...
    let mut r = BitReader { bytes, bit_len, pos };
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_map_dest_begin_update(process: *mut Process, base_ref_word: u64, extra: u32) -> u64 {
...
pub extern "C" fn fz_map_dest_freeze(process: *mut Process, dest_ref_word: u64) -> u64 {
    ...
    map_ref_word_from_bits((unsafe { &mut *process }).heap.map_destination_freeze(dest_bits))
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_ref(process: *mut Process, map_ref_word: u64, key_ref_word: u64) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_int_key_ref(process: *mut Process, map_ref_word: u64, value: i64) -> u64 {
    let map = any_value_ref_from_word(map_ref_word, "fz_map_get_int_key_ref map");
    fz_map_get_scalar_key_ref(process, map, AnyValue::int(value))
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_float_key_ref(process: *mut Process, map_ref_word: u64, value: f64) -> u64 {
...
pub extern "C" fn fz_map_count(map_ref_word: u64) -> i64 {
    ...
    unsafe { map_count(addr as *const u8) as i64 }
}
...
pub extern "C" fn fz_map_entry_key(map_ref_word: u64, index: i64) -> u64 {
    ...
    let (key, _) = unsafe { map_entry_refs(addr, index) };
    key.raw_word()
}
...
pub extern "C" fn fz_map_entry_value(map_ref_word: u64, index: i64) -> u64 {
    ...
    let (_, value) = unsafe { map_entry_refs(addr, index) };
    value.raw_word()
}

fn map_put_slot_value(process: *mut Process, map_ref_word: u64, key: AnyValue, value: AnyValue) -> u64 {
    let map_bits = map_bits_from_ref_word(map_ref_word, "map_put map");
    let new_map_bits = (unsafe { &mut *process }).heap.map_put_slot_bits(map_bits, key, value);
...
pub extern "C" fn fz_map_put_ref(
    ...
    if value.tag().is_scalar() {
        panic!("fz_map_put_ref value requires a heap/sentinel ref; use the typed scalar write path");
    }
    map_put_slot_value(
        process,
        map_ref_word,
        AnyValue::from_ref(key).expect("fz_map_put_ref key"),
        AnyValue::from_ref(value).expect("fz_map_put_ref value"),
    )
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_put_int(process: *mut Process, map_ref_word: u64, key_ref_word: u64, value: i64) -> u64 {
    let key = any_value_ref_from_word(key_ref_word, "fz_map_put_int key");
    map_put_slot_value(
        process,
        map_ref_word,
        AnyValue::from_ref(key).expect("fz_map_put_int key"),
        AnyValue::int(value),
    )
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_put_float(process: *mut Process, map_ref_word: u64, key_ref_word: u64, value: f64) -> u64 {
...
pub extern "C" fn fz_map_put_atom(process: *mut Process, map_ref_word: u64, key_ref_word: u64, atom_id: u64) -> u64 {
    ...
    map_put_slot_value(
        process,
        map_ref_word,
        AnyValue::from_ref(key).expect("fz_map_put_atom key"),
        AnyValue::atom(atom_id as u32),
    )
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_int(process: *mut Process, map_ref_word: u64, key_ref_word: u64) -> i64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_float(process: *mut Process, map_ref_word: u64, key_ref_word: u64) -> f64 {
    ref_load_float_impl(fz_map_get_ref(process, map_ref_word, key_ref_word))
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_map_get_atom(process: *mut Process, map_ref_word: u64, key_ref_word: u64) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_list_cons_ref(process: *mut Process, head_ref_word: u64, tail_ref_word: u64) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_list_cons_any(process: *mut Process, head_ref_word: u64, tail_ref_word: u64) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_list_cons_atom(process: *mut Process, atom_id: u64, tail_ref_word: u64) -> u64 {
...
pub extern "C" fn fz_list_head_ref(list_ref_word: u64) -> u64 {
    ...
    list_head_ref(list).expect("fz_list_head_ref").raw_word()
}
...
pub extern "C" fn fz_list_tail_ref(list_ref_word: u64) -> u64 {
    ...
    list_tail_ref(list).expect("fz_list_tail_ref").raw_word()
}
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_struct_get_field_ref(process: *mut Process, struct_ref_word: u64, field_offset: u32) -> u64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_struct_get_named_field_ref(process: *mut Process, value_ref_word: u64, field_atom_id: u64) -> u64 {
...
pub extern "C" fn fz_closure_get_capture_i64(closure_ref_word: u64, index: u64) -> i64 {
    ...
    let addr = value.closure_addr().expect("fz_closure_get_capture_i64 closure");
...
pub extern "C" fn fz_closure_get_capture_f64(closure_ref_word: u64, index: u64) -> f64 {
    ...
    let addr = value.closure_addr().expect("fz_closure_get_capture_f64 closure");
    match unsafe { closure_capture_value(addr, index as usize) } {
        AnyValue::Float(bits) => f64::from_bits(bits),
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_closure_set_capture_i64(process: *mut Process, closure_ref_word: u64, index: u64, value: i64) {
    let closure = any_value_ref_from_word(closure_ref_word, "fz_closure_set_capture_i64 closure");
    let addr = closure.closure_addr().expect("fz_closure_set_capture_i64 closure");
    unsafe {
        (&mut *process)
            .heap
            .write_closure_capture_value(addr, index as usize, AnyValue::Int(value))
    };
}

#[unsafe(no_mangle)]
pub extern "C" fn fz_closure_set_capture_f64(process: *mut Process, closure_ref_word: u64, index: u64, value: f64) {
    let closure = any_value_ref_from_word(closure_ref_word, "fz_closure_set_capture_f64 closure");
    let addr = closure.closure_addr().expect("fz_closure_set_capture_f64 closure");
    unsafe {
        (&mut *process)
            .heap
...
fn is_lazy_cont_ref(word: u64) -> bool {
    let tag_shift = AnyValueRefPacking::current().tag_shift();
    (word >> tag_shift) == TAG_FWD
}

#[inline]
fn lazy_cont_ptr(word: u64) -> *mut u8 {
    (word & AnyValueRefPacking::current().address_mask()) as *mut u8
}
...
unsafe fn lazy_cont_kind_base(ptr: *const u8, count: usize) -> *const u8 {
    unsafe { ptr.add(LAZY_CONT_RAW_OFF + count * size_of::<u64>()) }
}

#[inline]
unsafe fn lazy_cont_capture_raw(word: u64, index: usize) -> u64 {
    let ptr = lazy_cont_ptr(word);
    unsafe { *(ptr.add(LAZY_CONT_RAW_OFF + index * size_of::<u64>()) as *const u64) }
}
    ...
    for i in 0..count {
        ...
        match kind {
            ...
            LAZY_CONT_KIND_I64 => unsafe {
                (&mut *process)
                    .heap
                    .write_closure_capture_value(addr, i, AnyValue::Int(raw as i64))
            },
...
thread_local! {
    static FRAME_ALLOC_COUNT: Cell<u64> = const { Cell::new(0) };
}
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_alloc_frame(process: *mut Process, schema_id: u32, total_size: u32) -> *mut u8 {
    FRAME_ALLOC_COUNT.with(|c| c.set(c.get() + 1));
    ...
    if !process.is_null() {
        unsafe { &mut *process }
            .heap
            .record_external_alloc(HeapAllocKind::Frame, rounded);
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn fz_matcher_map_get_ref(process: *mut Process, map_ref_word: u64, key_ref_word: u64) -> u64 {
...
pub extern "C" fn fz_matcher_eq_bytes(val_bits: u64, bytes_ptr: u64, byte_len: u64) -> u32 {
    ...
    let got_bits = unsafe { bitstring_bit_len(p) };
    if got_bits != want_bits {
        return 0;
    }
    let val_ptr = unsafe { bitstring_byte_ptr(p) };
    let val_slice = unsafe { from_raw_parts(val_ptr, byte_len as usize) };
    let want_slice = unsafe { from_raw_parts(bytes_ptr as *const u8, byte_len as usize) };
