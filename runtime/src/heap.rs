...
impl Heap {
    ...
    pub fn alloc_struct(&mut self, schema_id: u32) -> *mut HeapHeader {
        ...
        unsafe {
            ...
            std::ptr::write((p as *mut u8).add(4) as *mut u32, 0);
            // Zero payload.
    ...
    fn alloc_vec_raw(&mut self, len: u64, payload_bytes: usize) -> *mut HeapHeader {
    ...
    pub fn alloc_vec_i64(&mut self, elements: &[i64]) -> *mut HeapHeader {
        ...
        unsafe {
            let payload = (p as *mut u8).add(8) as *mut i64;
            std::ptr::copy_nonoverlapping(elements.as_ptr(), payload, elements.len());
        }
        p
    }

    pub fn alloc_vec_f64(&mut self, elements: &[f64]) -> *mut HeapHeader {
        let p = self.alloc_vec_raw(elements.len() as u64, elements.len() * 8);
        unsafe {
            let payload = (p as *mut u8).add(8) as *mut f64;
            std::ptr::copy_nonoverlapping(elements.as_ptr(), payload, elements.len());
        }
        p
    }
    ...
    /// Read the element count from a strict tagged or legacy heap vec.
    ...
    pub fn vec_payload_ptr(p: *const HeapHeader) -> *const u8 {
        if (p as u64) & crate::fz_value::TAG_MASK != 0 {
            let addr = ((p as u64) & !crate::fz_value::TAG_MASK) as *const u8;
            return unsafe { crate::fz_value::vec_payload_ptr(addr) };
        }
        unsafe { (p as *const u8).add(24) }
    }
    ...
    pub fn gc_with_extra_roots(
        ...
        if !root_slot.is_null() {
            ...
            } else if let Some(p) = crate::fz_value::struct_addr_from_tagged(root_bits) {
                if ptr_in_from_space(p as *mut u8, &from_ranges)
                    || classify_fragment(p as *mut u8, &self.fragments).is_some()
                {
                    let schema_id = unsafe { crate::fz_value::struct_schema_id(p as *const u8) };
                    let size = crate::fz_value::struct_size_for_payload(
                        self.schemas.borrow().get(schema_id).size as usize,
                    );
                    let new_root = cheney_forward_tagged(
                        p,
                    ...
                    *root_slot =
                        crate::fz_value::tagged_struct_bits(new_root as *const u8) as *mut u8;
                }
            } else if let Some(p) = crate::fz_value::bitstring_addr_from_tagged(root_bits) {
                if ptr_in_from_space(p as *mut u8, &from_ranges)
                    || classify_fragment(p as *mut u8, &self.fragments).is_some()
                {
                    let new_root = cheney_forward_tagged(
            ...
            } else if let Some(p) = crate::fz_value::resource_addr_from_tagged(root_bits) {
                if ptr_in_from_space(p as *mut u8, &from_ranges)
                    || classify_fragment(p as *mut u8, &self.fragments).is_some()
                {
                    let new_root = cheney_forward_resource(
                        p,
                        &mut self.fragments,
                        &mut frag_queue,
                        &mut free,
                    ...
                    *root_slot =
                        crate::fz_value::tagged_resource_bits(new_root as *const u8) as *mut u8;
                }
            } else if let Some(p) = crate::fz_value::vec_addr_from_tagged(root_bits) {
                if ptr_in_from_space(p as *mut u8, &from_ranges)
                    || classify_fragment(p as *mut u8, &self.fragments).is_some()
                {
                    let tag = root_bits & crate::fz_value::TAG_MASK;
                    let new_root = cheney_forward_tagged(
                        p,
                        tag,
                        crate::fz_value::object_size(root_bits),
                    ...
                    *root_slot = ((new_root as u64) | tag) as *mut u8;
                }
            } else {
        ...
        for v in extra_roots.iter_mut() {
                    ...
                    || classify_fragment(p as *mut u8, &self.fragments).is_some())
            {
                ...
                *v = crate::fz_value::FzValue(crate::fz_value::tagged_struct_bits(
                    new_p as *const u8,
                ));
                continue;
            }
                    ...
                    || classify_fragment(p as *mut u8, &self.fragments).is_some())
            {
                ...
                *v = crate::fz_value::FzValue(crate::fz_value::tagged_bitstring_bits(
                    new_p as *const u8,
                ));
                continue;
            }
            ...
            if let Some(p) = crate::fz_value::resource_addr_from_tagged(v.0)
                ...
                && (ptr_in_from_space(p as *mut u8, &from_ranges)
                    || classify_fragment(p as *mut u8, &self.fragments).is_some())
            {
                let new_p = cheney_forward_resource(
                    p,
                    &mut self.fragments,
                    &mut frag_queue,
                    &mut free,
                ...
                *v = crate::fz_value::FzValue(crate::fz_value::tagged_resource_bits(
                    new_p as *const u8,
                ));
                continue;
            }
                    ...
                    || classify_fragment(p as *mut u8, &self.fragments).is_some())
            {
                ...
                *v = crate::fz_value::FzValue((new_p as u64) | tag);
                continue;
            }
...
fn cheney_forward_resource(
    ...
    if let Some(fwd) = is_forwarded_resource(p as *const u8) {
        return fwd as *mut HeapHeader;
    }
    let size = 32;
    let dst = *free;
...
fn cheney_trace_children(
    ...
    match kind {
        ...
        HeapKind::Resource => {
            // Legacy off-heap refcounted shared_ptr at +16 (handled by MSO sweep).
            // fz-4mk — legacy dtor closure FzValue at +24; trace like any other
...
pub fn deep_copy_value(
    ...
    if let Some(sp) = crate::fz_value::struct_addr_from_tagged(src.0)
        && !sp.is_null()
        && src_heap.contains_heap_addr(sp as *mut u8)
    {
        return deep_copy_strict_struct(sp, src_heap, dst_heap, forwarding);
    }
    if let Some(sp) = crate::fz_value::bitstring_addr_from_tagged(src.0)
        && !sp.is_null()
        && src_heap.contains_heap_addr(sp as *mut u8)
    {
        if let Some(&dp) = forwarding.get(&sp) {
        ...
        match f.kind {
            ...
            FieldKind::RawF64 | FieldKind::RawI64 | FieldKind::RawBytes(_) => unsafe {
                ...
                std::ptr::copy_nonoverlapping(
                    (sp as *const u8).add(8 + f.offset as usize),
                    (dp as *mut u8).add(8 + f.offset as usize),
                    width,
                );
            },
        }
...
mod tests {
    ...
    fn alloc_bitstring_inline_has_trailing_nul() {
        ...
        for n in [0usize, 1, 7, 8, 9, 15, 16, 17, 24, 25] {
            ...
            unsafe {
                assert_eq!(crate::fz_value::bitstring_bit_len(p as *const u8), bit_len);
                assert_eq!(
                    crate::fz_value::bitstring_size_for_bit_len(bit_len),
                    crate::fz_value::object_size(crate::fz_value::tagged_bitstring_bits(
                        p as *const u8
                    ))
                );
                let payload = crate::fz_value::bitstring_bytes_ptr(p as *const u8);
                for i in 0..n {
    ...
    fn alloc_vec_i64_writes_header_len_and_payload() {
        ...
        let tagged = crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_I64);
        assert_eq!(crate::fz_value::object_size(tagged), 32);
        assert_eq!(Heap::vec_len(tagged as *const HeapHeader), 3);
        unsafe {
            let payload = Heap::vec_payload_ptr(tagged as *const HeapHeader) as *const i64;
    ...
    fn alloc_vec_u8_packs_bytes() {
        ...
        let tagged = crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_U8);
        assert_eq!(crate::fz_value::object_size(tagged), 16);
        assert_eq!(Heap::vec_len(tagged as *const HeapHeader), 3);
        unsafe {
            let payload = Heap::vec_payload_ptr(tagged as *const HeapHeader);
    ...
    fn alloc_vec_bit_packs_msb_first() {
        ...
        let tagged = crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_BIT);
        assert_eq!(crate::fz_value::object_size(tagged), 16);
        assert_eq!(Heap::vec_len(tagged as *const HeapHeader), 7);
        unsafe {
            let payload = Heap::vec_payload_ptr(tagged as *const HeapHeader);
    ...
    fn alloc_large_struct_succeeds_and_grows_size_class() {
        ...
        unsafe {
            assert_eq!(crate::fz_value::struct_schema_id(p as *const u8), id);
    ...
    fn struct_layout_size_correct() {
        ...
        unsafe {
            assert_eq!(crate::fz_value::struct_schema_id(p as *const u8), id);
            assert_eq!(crate::fz_value::struct_flags(p as *const u8), 0);
        }
    }
    ...
    fn struct_field_read_at_new_offset() {
        ...
        unsafe {
            assert_eq!(
                std::ptr::read((p as *const u8).add(8) as *const FzValue).unbox_int(),
                Some(11)
            );
            assert_eq!(
                std::ptr::read((p as *const u8).add(16) as *const FzValue).unbox_int(),
                Some(22)
            );
        }
    ...
    fn struct_forwarding_marker_through_gc() {
        ...
        let mut h = Heap::new(SIZE_TABLE[0], reg);
        let p = h.alloc_struct(id);
        let old_addr = p as *const u8;
        h.write_field(p, 0, FzValue::from_int(9));
        let mut root = crate::fz_value::tagged_struct_bits(p as *const u8) as *mut u8;
    ...
    fn alloc_large_vec_i64_round_trips_through_gc() {
        ...
        let mut root =
            crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_I64) as *mut u8;
        h.gc(&mut root);
        assert_eq!(h.live_count(), 1);
    ...
    fn vec_f64_forwarding_marker_through_gc() {
        ...
        let mut root =
            crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_F64) as *mut u8;
        h.gc(&mut root);
        let new_p = root as *mut HeapHeader;
    ...
    fn vec_u8_forwarding_marker_through_gc() {
        ...
        let p = h.alloc_vec_u8(&[0xff, 0xab, 0x12]);
        let mut root =
            crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_U8) as *mut u8;
        h.gc(&mut root);
        let new_p = root as *mut HeapHeader;
        assert_eq!(Heap::vec_len(new_p), 3);
    ...
    fn vec_bit_forwarding_marker_through_gc() {
        ...
        let mut root =
            crate::fz_value::tagged_vec_bits(p as *const u8, ValueKind::VEC_BIT) as *mut u8;
        h.gc(&mut root);
        let new_p = root as *mut HeapHeader;
    ...
    fn alloc_bitstring_small_stays_inline() {
        ...
        let tagged = crate::fz_value::tagged_bitstring_bits(p as *const u8);
        unsafe {
            assert_eq!(
                tagged & crate::fz_value::TAG_MASK,
                crate::fz_value::TAG_BITSTRING
            );
            assert_eq!(bitstring_bit_len(tagged as *const HeapHeader), 256);
    ...
    fn fragment_to_block_edge_promotes_block_object() {
        ...
        let child_bits = unsafe { std::ptr::read((big as *const u8).add(8) as *const u64) };
