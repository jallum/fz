---
purpose: VR.5b — dbg dispatches to typed FFI when the arg Descr narrows
...
VR.5b — dbg dispatches to typed FFI when the arg Descr narrows

## Notes

Before fz-ul4.27.7, every `dbg(x)` went through `fz_dbg_value(boxed)`
...
to fz_dbg_value's render() output (4.0 stays "4.0", not "4") and
push to TEST_CAPTURE so cargo-test assertions keep working through
either entry point.

Polymorphic prints (arg type `any`) still route through fz_dbg_value
