...
| `cold_fn.fz` | minimal call site — one fn definition, one call, no scaffolding | jit |
...
| `hello.fz` | print each scalar shape — int, atom, bool, nil | jit |
| `higher_order.fz` | higher-order patterns — apply2, compose | jit |
| `hot_fn.fz` | same call repeated — historical JIT tier-up trigger; today every call is JIT | jit |
| `id_int_atom.fz` | identity fn over int and atom — no type-driven specialization | jit |
| `import.fz` | selective import — `import Math, only: [add: 2]` | jit |
| `interp_only_main.fz` | tiny module with a single helper and a main — historical interp-tier-0 smoke test | jit |
| `macro_inc.fz` | defmacro + quote/unquote round-trip — two macros, one nested in the other | jit |
| `modules.fz` | cross-module qualified calls — `M.double`, `M.quad`, `N.helper` | jit |
...
| `nested_modules.fz` | inner module addressed both fully-qualified (`Outer.Inner.f`) and via outer-local reference | jit |
| `polymorphic.fz` | parametric `id` exercised over int, atom, and bool | jit |
