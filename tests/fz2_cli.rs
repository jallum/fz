...
fn the_drain_arbiter_publishes_readiness_only_movement_and_attributes_every_evaluation() {
    ...
    for wake in &wakes {
        ...
        assert!(
            cause.get("use").and_then(serde_json::Value::as_str) == Some("settled")
                && matches!(cause_kind, "ActivationAnalyzed" | "CallSiteSummary")
                && job.get("kind").and_then(serde_json::Value::as_str) == Some("DeriveExecutableFacts")
                && ["root_id", "function_id", "arrow"]
                    .into_iter()
                    .all(|field| cause.get(field) == job.get(field))
                && wake.get("shift").and_then(serde_json::Value::as_bool) == Some(false),
            "only settled ActivationAnalyzed/CallSiteSummary facts may wake their exact \
             DeriveExecutableFacts producer here: {wake:?}"
        );
    ...
    assert_eq!(
        ...
        FormulaWork {
            ...
            uncaused: 0,
            changed_outputs: 188,
            unchanged_outputs: 120,
            wakes: 127,
            blocked_completions: 133,
        },
